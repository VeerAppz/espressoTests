...
import com.example.flipkart.interfaces.GlobalVariables;
...
//            eventInitilizer.sendAllEventOfEndOfTest();
    ...
    @Test
    public void FlipkartLaunchTests() {
        String testCaseName = "Launch Test for demo flipkart";
        try {
            assertEquals("Launching App Failed", true, coldLaunch(testCaseName));
            assertEquals("Home Element Appear Failed", true, FlipkartHomeElementAppear(testCaseName));
            assertEquals("Click On Search Button Failed", true, FlipkartClickSearchButton(testCaseName));
            assertEquals("Enter Search Text and Search a Product Failed", true, FlipkartSearchProduct(testCaseName));
            assertEquals("Search Result Appear Failed", true, FlipkartSearchResultAppear(testCaseName));
            assertEquals("Click On Search Result Failed", true, FlipkartClickOnSearchResult(testCaseName));
            Thread.sleep(5000);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//            context.startActivity(intent);
    ...
    private boolean FlipkartHomeElementAppear(String testCaseName) {
        boolean value = false;
        try {int i=0;
    ...
    private boolean FlipkartClickSearchButton(String testCaseName) {
        boolean value = false;
        try {int i=0;
    ...
    private boolean FlipkartSearchProduct(String testCaseName) {
        boolean value = false;
        try {int i=0;
    ...
    private boolean FlipkartSearchResultAppear(String testCaseName) {
        boolean value = false;
        try {int i=0;
    ...
    private boolean FlipkartClickOnSearchResult(String testCaseName) {
        boolean value = false;
        try {int i=0;
            UiObject object = device.findObject(new UiSelector().textContains("SAMSUNG Galaxy S23 Ultra 5G (Phantom Black, 512 GB)"));
//            UiScrollable scrollable = new UiScrollable(new UiSelector().scrollable(true));
